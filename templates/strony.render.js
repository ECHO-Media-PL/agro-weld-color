...
(function (root) {
  ...
  function realCard(r, rel) {
    return '<a class="rz" href="/realizacje/' + r.slug + '/" style="background:#ECE7D7;display:flex;flex-direction:column;color:#14160E;text-decoration:none">' +
      ...
      '<span class="mono" style="margin-top:auto;font-size:11px;font-weight:700;color:#41571F">Zobacz case study <span class="rzarr">↗</span></span></span></a>';
  }
  function renderRealizacje(list, tpl) {
    var body = heroLight({ crumbs: [['Start', '/'], ['Realizacje']], k1: 'Realizacje', k2: list.length + ' wdrożenia', h1: 'Wdrożone linie <span style="color:#3B5115">i maszyny u klientów</span>', lead: 'Każda realizacja zaczyna się od surowca, skali produkcji i układu hali klienta. Poniżej wybrane wdrożenia — od pojedynczych stanowisk po kompletne linie pod klucz.' }) +
  ...
  function renderCase(r, list, machines, tpl) {
    ...
    return shell(tpl, { depth: 2, url: 'https://www.agro-weld.pl/realizacje/' + r.slug + '/', title: r.title + ' – realizacja Agro-Weld', desc: r.lead, active: 'realizacje', body: body,
      css: '.rz{transition:background .3s}.rz:hover{background:#F4EFE2}.rz:hover .rzimg{transform:scale(1.05)}.rzimg{transition:transform .6s cubic-bezier(.2,.7,.2,1)}.rzarr{display:inline-block;transition:transform .3s}.rz:hover .rzarr{transform:translate(3px,-3px)}.relm{transition:background .25s}.relm:hover{background:#F4EFE2}.relm:hover .rarr{transform:translate(3px,-3px)}.rarr{display:inline-block;transition:transform .3s}@media(max-width:900px){.rzgrid{grid-template-columns:1fr!important}}' });
  }
